import { Circuit, Field, Ledger, Pickles, } from '../snarky';
import { cloneCircuitValue } from './circuit_value';
import { Body, Party, signJsonTransaction, Permissions, } from './party';
import { PrivateKey } from './signature';
import * as Mina from './mina';
import { UInt32, UInt64 } from './int';
import { withContext, getContext, mainContext, } from './global-context';
import { assertPreconditionInvariants, cleanPreconditionsCache, } from './precondition';
export { deploy, signFeePayer, declareMethods };
 ...
 * ```
 */
export function method(target, methodName, descriptor) {
    ...
    for (let i = 0; i < paramTypes.length; i++) {
        ...
        if (isProof(Parameter)) {
            ...
            throw Error(`Argument ${i} of method ${methodName} is not a valid circuit value.`);
        }
    }
...
function wrapMethod(method, argsLength, ZkappClass) {
    function wrappedMethod(...args) {
        let actualArgs = args.slice(0, argsLength);
        let shouldCallDirectly = args[argsLength];
...
function isAsFields(typ) {
    return (!!typ && ['toFields', 'ofFields', 'sizeInFields'].every((s) => s in typ));
}
function isProof(typ) {
    return false; // TODO
}
function toStatement(self, tail) {
...
function checkStatement([transaction, atParty], self, tail) {
...
function picklesRuleFromFunction(identifier, func, witnessTypes) {
    function main(statement, previousStatements) {
        let { self, witnesses } = getContext();
        witnesses = witnessTypes.map(witnesses
...
export class SmartContract {
    ...
    static async compile(address) {
        ...
        address ?? (address = PrivateKey.random().toPublicKey());
        let instance = new this(address);
        let rules = (this._methods ?? []).map(({ methodName, witnessArgs }) => picklesRuleFromFunction(methodName, (...args) => instance[methodName](...args), witnessArgs));
        let [, { getVerificationKeyArtifact, provers, verify }] = withContext({ self: selfParty(address), inCompile: true }, () => Pickles.compile(rules, 2));
...
async function deploy(SmartContract, { zkappKey, verificationKey, initialBalance, shouldSignFeePayer, feePayerKey, transactionFee, feePayerNonce, memo, }) {
    let address = zkappKey.toPublicKey();
    let tx = Mina.createUnsignedTransaction(() => {
        if (initialBalance !== undefined) {
            if (feePayerKey === undefined)
                throw Error(`When using the optional initialBalance argument, you need to also supply the fee payer's private key feePayerKey to sign the initial balance funding.`);
            ...
            let nonce = feePayerNonce !== undefined
    ...
    if (shouldSignFeePayer) {
        if (feePayerKey === undefined || transactionFee === undefined) {
            throw Error(`When setting shouldSignFeePayer=true, you need to also supply feePayerKey (fee payer's private key) and transactionFee.`);
        }
